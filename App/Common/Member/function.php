...
function getTenderList($map,$size,$limit=10){
 ...
 function getInvestList($map,$size,$limit=10){
	$pre = C('DB_PREFIX');
	$Bconfig = require C("APP_ROOT")."Conf/bill_config.php";
	
	if(empty($map['invest_uid'])) return;
	if($size){ 
		//分页处理
		import("ORG.Util.Page");
		$count = M('bill_invest i')->join("{$pre}bill bi ON bi.id = i.bill_id")->where("i.invest_uid = {$map['invest_uid']} and bi.status={$map['status']}")->count('i.id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	...
	$type_arr =$Bconfig['BORROW_TYPE'];
	/////////////////////////视图查询 fan 20130522//////////////////////////////////////////
	//$list=M('bill b')->join("{$pre}bill_invest bi ON bi.bill_id = b.id")->field("b.id bid,b.*,bi.*")->where("bi.invest_uid = {$map['invest_uid']} and b.status = {$map['status']}")->group('id')->limit($Lsql)->select();
	$list=M('bill b')->join("{$pre}bill_invest bi ON bi.bill_id = b.id")->field("b.id bid,b.*,bi.invest_amount,bi.invest_interest,bi.id")->where("bi.invest_uid = {$map['invest_uid']} and b.status = {$map['status']} ")->limit($Lsql)->select();
	
	$row=array();
	$row['list'] = $list;
	$row['page'] = $page;
	$row['total_money'] = M('bill_invest i')->join("{$pre}bill bi ON bi.id = i.bill_id")->where("i.invest_uid = {$map['invest_uid']} and bi.status={$map['status']}")->sum('invest_amount');
	$row['total_num'] = $count;
	return $row;
}
...
//////////////////////////////企业直投 管理模块结束  /////////////////////////////
	function getLoanList($map,$size,$limit=10){
		if(empty($map['loan_uid'])) return;
	
		if($size){
			//分页处理
			import("ORG.Util.Page");
			$count = M('Loan')->where($map)->count('id');
			$p = new Page($count, $size);
			$page = $p->show();
			$Lsql = "{$p->firstRow},{$p->listRows}";
			//分页处理
		...
		$list=M('Loan')->field(true)->where($map)->group('id')->limit($Lsql)->select();
	...
	function getLoaninvestList($map,$size,$limit=10){
		$pre = C('DB_PREFIX');
		$Bconfig = require C("APP_ROOT")."Conf/bill_config.php";
		
		if(empty($map['li.invest_uid'])&&empty($map['l.id'])) return;
		if($size){ 
			//分页处理
			import("ORG.Util.Page");
			$count = M('loan_invest li')->join("{$pre}loan l ON l.id = li.loan_id")->where($map)->count('li.id');
			$p = new Page($count, $size);
			$page = $p->show();
			$Lsql = "{$p->firstRow},{$p->listRows}";
			//分页处理
		...
		$list=M('loan_invest li ')
				->join("{$pre}loan l ON li.loan_id = l.id")
				->join("{$pre}members m ON li.invest_uid = m.id")
				->field("l.id as lid,l.status,l.collect_time,l.begin_time,l.loan_type,l.loan_name,l.interest_rate,l.loan_duration,l.duration_type,li.*,m.user_name")
				->where($map)->limit($Lsql)->select();
		
		$row=array();
		$row['list'] = $list;
		$row['page'] = $page;
		$row['total_money'] = M('loan_invest li')->join("{$pre}loan l ON l.id = li.loan_id")->where($map)->sum('invest_amount');
		$row['total_num'] = $count;
		return $row;
	}
?>