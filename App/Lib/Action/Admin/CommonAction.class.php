...
//----------配资通知开始--------------	
	public function peizisms(){
		$utype = C('XMEMBER_TYPE');
        if(in_array(intval($_GET['tab']), array(1,2,3,4)))  $tab = intval($_GET['tab']);
        else    $tab = 1;
        $this->assign("tab", $tab);
        $this->assign("user_name", text($_GET['user_name']));
		$this->assign("borrow_money", text($_GET['borrow_money']));
        $this->assign("admin_id", $this->admin_id);
        $this->display();
    }
...
//-----------配资通知结束-------------
    ...
    public function sendsms(){
		if(intval($_POST['sms'])==0){
			
			$user_name = text($_POST['user_name']);
			$user = M('members m')->field(" m.id,mi.real_name,m.user_email,m.user_phone,ms.email_status,ms.phone_status ")
            ...
            $datas[]=$user['real_name'];
           			
			if($title=="账号通知"){
				$uname =text($_POST['uname']);
			    $pwd =text($_POST['pwd']);
			    if ($uname == "") 	exit("账号不可为空");
			    if ($pwd == "") 	exit("密码不可为空");
				$content="【百财网】亲爱的".$user['real_name']."，您的操盘账号已分配，请使用涌金版客户端进行操作，交易账号:".$uname."，密码是".$pwd."，祝您炒股愉快！";
				$tempId=20536;
				$datas[]=$uname;
				$datas[]=$pwd;
			}elseif($title=="追保提醒"){
				$content="【百财网】亲爱的".$user['real_name']."，您的股票账户已接近预警线，请做好明天追加保证金的准备以便您更好的操作，谢谢，祝您投资愉快";
				
				$tempId=21141;	
				
			}elseif($title=="到期提醒"){
				$content="【百财网】亲爱的".$user['real_name']."，您的配资将于明天到期，请保证交易账户已平仓，谢谢合作！";
