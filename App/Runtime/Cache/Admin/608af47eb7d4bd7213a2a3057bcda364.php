...
<!--搜索/筛选会员-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
		...
		<td>
            <a onclick="loadUser(<?php echo ($vo["recommend_id"]); ?>,'<?php echo ($vo["recommend_name"]); ?>')" href="javascript:void(0);"><?php echo ($vo["recommend_name"]); ?></a>
            <?php if(empty($vo["recommend_id"])): ?><a href="javascript:;" onclick="showurl('__URL__/recommendedit?id=<?php echo ($vo['id']); ?>','添加推荐人');">[手添]</a><?php endif; ?>
        </td>
