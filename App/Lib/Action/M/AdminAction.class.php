<?php
    ...
    {	
        ...
        public function total(){
			...
			$map_account['uid']=array("not in","106");
			$map_account['account_money']=array('egt',10000);
			$investable =M('member_money')->where($map_account)->count();
			$map_loan_sum['status']=array('in','2,7,8');;
			$loan_sum=M('loan')
				->field('status,count(id) as loan_num,sum(has_collect) as has_collect,sum(loan_amount) as loan_amount')
				->group('status')
				->where($map_loan_sum)
				->select();
			foreach ($loan_sum as $k=> $v) {
				$v['need']=num_format($v['loan_amount']-$v['has_collect']);
				$v['loan_amount']=($v['loan_amount']/10000).'万';
				$loan_sumx[$v['status']]=$v;
			}
			$this->assign('loan_sum',$loan_sumx);
