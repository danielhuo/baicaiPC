...
class VerifyAction extends MCommonAction {
    ...
    public function idcard(){
		...
		if($ids==1){
			$vo = M("member_info")->field('idcard,real_name,card_img,card_back_img,card_ren_img,up_time')->find($this->uid);
			$vo['up_time'] = date('Y-m-d H:i:s',$vo['up_time']);
			$this->assign("vo",$vo);
			$data['html'] = $this->fetch();
		}
    ...
    public function saveid(){
		...
		//$isimg3 = session('idcardimg3');
		$data['real_name'] = text($_POST['realname']);
		$data['idcard'] = text($_POST['idcard']);
		$data['up_time'] = time();
		$id = M('member_info')->where("idcard= {$data['idcard']}")->count();
		if($id) ajaxmsg("身份证已注册",0);
		/////////////////////////保存name_apply
		$data1['idcard'] = text($_POST['idcard']);
		$data1['up_time'] = time();
		$data1['uid'] = $this->uid;
		$data1['status'] = 1;
		$id = M('name_apply')->where("idcard = {$_POST['idcard']}")->count('idcard');
		...
		//if($isimg3!=1) ajaxmsg("手持身份证正面照（本人照）",0);
    ...
    /*public function sendphone(){
		...
		if($is_manual==0){//如果未开启后台人工手机验证，则由系统向会员自动发送手机验证码到会员手机，
			//$res = sendsms($phone,str_replace(array("#UserName#","#CODE#"),array(session('u_user_name'),$code),$smsTxt['verify_phone']));
			$res=sendsms($phone,array(session('u_user_name'),$code,1),18723);
	...
	public function ajaxupimg3(){
		if(!empty($_FILES['imgfile3']['name'])){
			$this->fix = false;
			$this->saveRule = date("YmdHis",time()).rand(0,1000)."_{$this->uid}_ren";
			$this->savePathNew = C('MEMBER_UPLOAD_DIR').'Idcard/' ;
			$this->thumbMaxWidth = "1000,1000";
			$this->thumbMaxHeight = "1000,1000";
			$info = $this->CUpload();
		...
		if($img){
			$c = M('member_info')->where("uid = {$this->uid}")->count('uid');
			if($c==1){
				$newid = M("member_info")->where("uid={$this->uid}")->setField('card_ren_img',$img);
			}else{
				$data['uid'] = $this->uid;
				$data['card_ren_img'] = $img;
				$newid = M('member_info')->add($data);
			}
			session("idcardimg3","1");
			ajaxmsg('',1);
		}
		else  ajaxmsg('',0);
	}
	...
	public function validId(){
		$id_card =text($_POST['idcard']);
		$id = M('member_info')->where("idcard= {$id_card}")->count();
		if($id!=0) ajaxmsg('',1);
		else ajaxmsg('',0);
