...
class WelcomeAction extends ACommonAction {
    ...
    public function index(){
		...
		$row['transferring'] = M('borrow_info')->where('borrow_status=6 and has_transfer=0')->count('id');//划款
		$row['depositadding'] = M('borrow_info')->where('borrow_status=6 and has_transfer=1 and deposit_addition>0')->count('id');//追保
	    $row['debting'] = M('invest_detb')->where('status=99')->count('id');//债权转让
		...
		$row['bill_1'] = M('bill')->where("status = 0")->count('id');
		$row['bill_2'] = M('bill')->where("status = 4")->count('id');
		$row['bill_3'] = M('bill')->where("status = 6")->count('id');
		$row['loan_1'] = M('loan')->where('status = 0')->count('id');
		$row['loan_2'] = M('loan')->where('status = 4')->count('id');
		$row['loan_3'] = M('loan')->where('status = 6')->count('id');
		
		
		$condition['status'] = 2;
		$condition['collect_time'] = array("lt",strtotime("now"));
		$row['loan_4'] = M('loan')->where($condition)->count('id');
	    
		
		$owl['status']=7;
		$owl['deadline']=array("elt",strtotime(date("Y-m-d"))+86400*7);
		$row['oneweekleft'] = M('bill')->where($owl)->count('id');
		$row['oneweekleft1'] = M('loan')->where($owl)->count('id');
		
		$row['accesstel1'] = M('accesstel')->where("status=0 and type=1")->count('id');
		$row['accesstel2'] = M('accesste2')->where("status=0 and type=2")->count('id');
