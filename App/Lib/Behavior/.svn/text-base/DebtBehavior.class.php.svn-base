       ...
       */
        ...
        */
        ...
        {  
		    ...
		    $isGudong=M('Gudong')->where("uid = ".$this->uid)->count();
			if($isGudong>0)
				$timeLimit=0;
			
			$count = M('borrow_investor')
			       ->where("investor_uid = ".$this->uid."  and status = 4 and debt_status=0 and ".time()."-birth_time > ".$timeLimit)
				   ->count();
        ...
        */
        private function checkSell($invest_id, $price,$fee,$paypass)
        {
            $condition = "invest_id= '".$invest_id."' and status in (6,7)";
        ...
        */
        ...
        {  
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id,d.id, i.borrow_id, d.money, d.transfer_price, d.buy_time, d.status, m.user_name,  
        ...
        */
        public function buyDetb()
        {
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id,d.id, i.borrow_id, d.money, d.transfer_price, d.buy_time, d.status, d.serialid, m.user_name,  
                            d.cancel_times, d.period, d.total_period, b.borrow_name, b.borrow_interest_rate, b.total, b.has_pay")
        ...
        */
        public function listAll($parm,$size=1000, $pstatus=true)
        {
            ...
            $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,
                     i.investor_uid, i.deadline, b.id, b.borrow_name, b.borrow_interest_rate,b.borrow_status,b.borrow_duration,m.credits, m.user_name, b.borrow_uid as uid";
            if($pstatus){ 
        ...
        */
		...
		//买方利息入余额 投资款入待收
		public function buy($paypass, $invest_id)
        {   $invest_info=M('Borrow_investor')->field("investor_capital,investor_interest")
		                                      ->where("id=".$invest_id)->find();
            $check_result = $this->checkBuy($paypass, $invest_id);
            if($check_result === 'TRUE'){
                $debt_info = $this->Model->field("transfer_price,transfer_fee,sell_uid, money")->where("invest_id={$invest_id}")->find();
                $buy_user = M("member_money")->field("account_money, back_money, money_collect")->where("uid={$this->uid}")->find();
                $sell_user = M("member_money")->field("account_money, money_collect")->where("uid={$debt_info['sell_uid']}")->find();
                if($debt_info['transfer_price'] > ($buy_user['account_money']+$buy_user['back_money'])){
                    return 'insufficient_account_balance';
				...
				//买方待收余额增加值=投资款+利息
                //$buy_user['money_collect'] += $debt_info['money']; 
				  $buy_user['account_money'] += $invest_info['investor_interest'];
				  $buy_user['money_collect'] += $invest_info['investor_capital'];
                //转让方增加资金
                $sell_user['account_money'] += $debt_info['transfer_price'];
				
                //$sell_user['money_collect'] = $sell_user['money_collect'] - $debt_info['money'];
				$sell_user['money_collect'] -= $invest_info['investor_capital'];// 减去待收
                $sell_user['account_money'] -= $invest_info['investor_interest'];// 减去利息
                ...
                if($investor_status && $detail_status && $debt_status && $sell_user_status && $buy_user_status){
                    ...
                    $this->moneyLog($this->uid, 46, -$debt_info['transfer_price'], $debt_info['money'], "购买{$serialid}号债权", $debt_info['sell_uid']);
                    $this->moneyLog($debt_info['sell_uid'], 47, $debt_info['transfer_price'], $debt_info['money'], "转让{$serialid}号债权", $this->uid);
                    memberMoneyLog($debt_info['sell_uid'],48,-$debt_info['transfer_fee'],"转让{$serialid}号债权手续费");
					memberMoneyLog(106,48,$debt_info['transfer_fee'],"获得{$serialid}号债权转让手续费");
                    //return 'TRUE';  
					return '购买成功';
                }else{
                    $this->Model->rollback();
