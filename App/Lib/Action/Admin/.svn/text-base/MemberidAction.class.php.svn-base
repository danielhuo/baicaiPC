...
class MemberidAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$list = M('members_status s')->field('s.uid,s.id_status,mi.up_time,mi.card_img,mi.card_back_img,mi.card_ren_img,mi.real_name,mi.idcard,m.user_name')->join("{$pre}members m ON m.id=s.uid")->join("{$pre}member_info mi ON mi.uid=s.uid")->where($map)->order("mi.up_time DESC")->limit($Lsql)->select();
	...
	public function export(){
		...
		//$page = $p->show();
		//$Lsql = "{$p->firstRow},{$p->listRows}";
		$list = M('members_status s')->field('s.uid,s.id_status,mi.up_time,mi.card_img,mi.card_back_img,mi.card_ren_img,mi.real_name,mi.idcard,m.user_name')->join("{$pre}members m ON m.id=s.uid")->join("{$pre}member_info mi ON mi.uid=s.uid")->where($map)->order("mi.up_time DESC")->select();//->limit($Lsql)
	...
	public function doEdit(){
		...
		if($status==1){
			...
			$vo= M("name_apply")->where("uid={$uid}")->find($id);
			addInnerMsg($vo['uid'],"您的实名已认证通过","您实名的认证已通过，处理说明：".$deal_info);
			sendsms($minfo['user_phone'],$minfo['user_name'],19494);
		
		}else{
		    			
			$new=M("name_apply")->where("uid={$uid}")->save($data);
			$data2['idcard']="";
			$midcard=M("member_info")->where("uid={$uid}")->save($data2);
			$vo= M("name_apply")->where("uid={$uid}")->find($id);
			addInnerMsg($vo['uid'],"您实名的认证未通过","您的实名认证未通过，处理说明：".$deal_info);
			sendsms($minfo['user_phone'],$minfo['user_name'],19495);
		}
		if($newxid){
