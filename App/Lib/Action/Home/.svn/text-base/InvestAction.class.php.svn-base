    ...
    */
    public function index()
    {
       ...
       $urlArr = array('borrow_status','borrow_duration','leve');
		...
		$searchMap['borrow_status'] = array("all"=>"不限制","2"=>"进行中","7"=>"已完成");
		$searchMap['borrow_duration'] = array("all"=>"不限制","1-2"=>"1-2个月","3-6"=>"3-6个月");
		...
		if($search['b.borrow_status']==0){
			...
			$search['b.borrow_status']=array("in","2,6,7");
		}
		...
		if($_GET['orderby']){
			...
			else $parm['orderby']="b.borrow_interest_rate DESC";
		}else{
    ...
    public function detail(){
		...
		//合同ID
		if($this->uid){
			$invs = M('borrow_investor')->field('id')->where("borrow_id={$id} AND (investor_uid=".$invest_id.")")->find();
		...
		//investinfo
		$fieldx = "bi.investor_capital,bi.add_time,m.user_name,bi.is_auto,bi.id";
		...
		$this->assign("investinfo2",$investinfo);
		//debtmoney
		...
		//是否实名认证
		$mstatus= M('members_status')->field('id_status,phone_status')
		                          ->where('uid='.$this->uid)->find();
