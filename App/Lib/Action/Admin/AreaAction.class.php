...
class AreaAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field= 'id,name,sort_order,reid,is_open,domain';
		$this->_list(M('area'),$field,array("reid"=>$reid),'sort_order');
        $this->display();
    }
    ...
    public function listType()
    {
		...
		$list.='<tr overstyle="on" id="list_'.$v['id'].'" class="leve_'.$leve.'" typeid="'.$v['id'].'" parentid="'.$v['reid'].'">
				<td class="text-center"><input type="checkbox" name="checkbox" id="checkbox2" onclick="checkon(this)" value="'.$v['id'].'"></td>
				<td>'.$v['id'].'</td>
				...
				<td>
					...
					<a href="/admin/area/add?id='.$v['id'].'">添加子栏目</a>  
				</td>
			  </tr>';
		}
