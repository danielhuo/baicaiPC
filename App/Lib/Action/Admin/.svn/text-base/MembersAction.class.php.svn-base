...
class MembersAction extends ACommonAction
{
    ...
    */
    ...
    public function doRecommendEdit()
    {
		$id = intval($_POST['id']);
		$uid = $id;
		$recName = text($_POST['user_name']);
		$recId=M('Members')->getFieldByUserName($recName);
		$done=false;
		if($recId){
			$condition['id']=$uid;
			$condition['recommend_id']=$recId;
			$done=M('Members')->save($condition);
		}
		//记录
		
        $this->assign('jumpUrl', __URL__."/index".session('listaction'));
		if($done){
			alogs("Members",0,1,'成功执行了会员添加推荐人的操作！');//管理员操作日志
			$this->success("操作成功");
		}else{
			alogs("Members",0,0,'执行会员添加推荐人的操作失败！');//管理员操作日志
			$this->error("操作失败");
		}
    }
   /*
